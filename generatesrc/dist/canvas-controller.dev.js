...
var lastname = '';
var firstrender = true; // draw line
...
function drawboard() {
  ...
  while (x < boardSize * 2 - 1) {
    ...
    while (y < boardSize * 2 - 1) {
      ...
      while (ed < eraseddots.length - 1) {
        // ok -1 is for some bug but yeah thats fine
        ctx.fillStyle = limecolor;
        var xc = eraseddots[ed][0];
        var yc = eraseddots[ed][1];
        xc = xc.replace('a', '10');
        xc = xc.replace('b', '11');
        xc = xc.replace('c', '12');
        xc = xc.replace('d', '13');
        ...
        if (Math.abs(basex + xc * _byte - (actx + _byte / 2 + _byte / 20)) < _byte / 4 && Math.abs(yc * _byte - (acty + _byte / 2 + _byte / 20)) < _byte / 4) {
          deactivated = true;
        }
...
var openspace = window.innerWidth / 2;
openspace = (openspace - _byte * (boardSize + 2)) / 2;
cvs.style.left = openspace + 'px';
cvs.style.top = _byte * 0.5 + 'px';
var testurl = window.location.href; //testurl = 'https://skparab1.github.io/pacman/generatesrc/?7.51,12.6,u&n=14.5,4.87,0,-1.5&n=7.49,12.4,1.56,0&n=9.53,10.5,0,0&n=10.5,10.5,0,0&n=3.5,a.5;3.5,b.0;3.5,b.5;3.5,c.0;3.5,c.5;4.0,c.5;4.5,c.5;5.0,c.5;5.5,c.5;6.0,c.5;6.5,c.5;7.0,c.5;7.5,c.5;8.0,c.5;8.5,c.5;9.0,c.5;9.5,c.5;a.0,c.5;a.5,c.5;b.0,c.5;b.5,c.5;c.0,c.5;c.5,c.5;d.0,c.5;d.5,c.5;d.5,d.0;d.5,d.5;d.5,e.0;d.5,e.5;d.0,e.5;c.5,e.5;c.5,f.0;c.5,f.5;c.5,g.0;c.5,g.5;c.0,g.5;b.5,g.5;b.0,g.5;a.5,g.5;a.0,g.5;9.5,g.5;9.0,g.5;8.5,g.5;8.0,g.5;7.5,g.5;7.0,g.5;6.5,g.5;6.5,g.0;6.5,f.5;6.5,f.0;6.5,e.5;7.0,e.5;7.5,e.5;7.5,e.0;7.5,d.5;7.5,d.0;&n=pink,red,orange,teal';

testurl = testurl.replace('https://skparab1.github.io/pacman/generatesrc/?', '');
testurl = testurl.replace('https://skparab1.github.io/pacman/generatesrc?', '');
var parts = testurl.split('&n=');
var pacpos = parts[0].split(',');
console.log(pacpos);
thepos = [basex + _byte * parseFloat(pacpos[0]), _byte * parseFloat(pacpos[1])];
dir = pacpos[2];
var ghost1 = parts[1].split(',');
g1pos = [basex + _byte * parseFloat(ghost1[0]), _byte * parseFloat(ghost1[1])];
g1dir = [parseFloat(ghost1[2]), parseFloat(ghost1[3])];
var ghost2 = parts[2].split(',');
g2pos = [basex + _byte * parseFloat(ghost2[0]), _byte * parseFloat(ghost2[1])];
g2dir = [parseFloat(ghost2[2]), parseFloat(ghost2[3])];
var ghost3 = parts[3].split(',');
g3pos = [basex + _byte * parseFloat(ghost3[0]), _byte * parseFloat(ghost3[1])];
g3dir = [parseFloat(ghost3[2]), parseFloat(ghost3[3])];
var ghost4 = parts[4].split(',');
g4pos = [basex + _byte * parseFloat(ghost4[0]), _byte * parseFloat(ghost4[1])];
g4dir = [parseFloat(ghost4[2]), parseFloat(ghost4[3])];
var gottendots = parts[5].split(';');
var idek = 0;

while (idek < gottendots.length) {
  var subj1 = gottendots[idek].split(',');
  eraseddots.push(subj1);
  idek += 1;
}

var ghclr = parts[6].split(',');
...
tmpLink.href = imageData; // temporarily add link to body and initiate the download  
//window.open(tmpLink);
