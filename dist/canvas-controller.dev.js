...
var avgfps = 0;
var fpslst = []; // we dont talk abt this
...
function drawboard() {
  ...
  ctx.strokeStyle = limecolor;
  ctx.strokeRect(window.innerWidth / 4 + _byte * 2, _byte * 13, _byte * 1, _byte * 1); // green dot on intersections
  // let cr = 0;
...
while (ctr < intersectionpre.length) {
  ...
  _subjarr4.push((intersectionpre[ctr][0] + 0.40) * _byte + window.innerWidth / 4);

  _subjarr4.push((intersectionpre[ctr][1] - 0.40) * _byte + window.innerWidth / 4);

  _subjarr4.push((intersectionpre[ctr][2] + 0.40) * _byte);

  _subjarr4.push((intersectionpre[ctr][3] - 0.40) * _byte);
...
(function _callee() {
  var counter, ct, rejected1, _ct, _rejected, _ct2, _rejected2, _ct3, _rejected3, dotchecker, z1, z2, randnotif, renderellapse, sum, _avgfps, lps, deviation, newspeed, openspace, result, ct3, ct11, rejected, _ct4, _rejected4, _ct5, _rejected5, _ct6, _rejected6, z3;
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 1:
          ...
          console.log(waiter); // pac man moving algorithm
          // upgrader updater
          ...
          drawghost(g4pos[0], g4pos[1], height / (boardSize * 2.2) * 0.75, 'teal'); // ghost mover for gh1

          result = moveghost(g1pos, g1dir, g1timer);
          g1pos = result[0];
          g1dir = result[1];
          g1timer = result[2]; // ghostmover for ghost 2

          result = moveghost(g2pos, g2dir, g2timer);
          g2pos = result[0];
          g2dir = result[1];
          g2timer = result[2]; // move ghost 3

          result = moveghost(g3pos, g3dir, g3timer);
          g3pos = result[0];
          g3dir = result[1];
          g3timer = result[2]; // move ghost 4

          result = moveghost(g4pos, g4dir, g4timer);
          g4pos = result[0];
          g4dir = result[1];
          g4timer = result[2]; // if pacman is stuck in one pos for some time then kick it off in a random dir
